...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
    ...
    //add current set kernel4.4
	retval= of_property_read_u32(np, "synaptics,vcc_i2c-current", &bdata->regulator_i2cvdd_current);
	if (retval < 0)
		dev_err(dev, "do not get i2c-vdd current \n");
		...
		const struct i2c_device_id *dev_id)
{
    ...
    printk("i2c.c synaptics_rmi4_i2c_probe \n");
