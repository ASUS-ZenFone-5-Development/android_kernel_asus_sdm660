...
//extern int   mdss_rst_ctl(int on);


#define _GESTURE_SLIDE 0x001
#define _GESTURE_DOUBLE_CLICK 0x002

#define _GESTURE_V 0x104
#define _GESTURE_Z 0x108
#define _GESTURE_C 0x110
#define _GESTURE_E 0x120
#define _GESTURE_S 0x140
#define _GESTURE_W 0x180

static  uint16_t gesture_key_array[] = {
		KEY_C,  	//GESTURE_WORD_C
		KEY_W,  	//GESTURE_WORD_W
		KEY_V,  	//GESTURE_WORD_V
		KEY_F23,  //GESTURE_DOUBLE_CLICK
		KEY_Z,  	//GESTURE_WORD_Z
		KEY_POWER,  //GESTURE_WORD_M
		KEY_POWER,  //GESTURE_WORD_O
		KEY_E,  	//GESTURE_WORD_e
		KEY_S,  	//GESTURE_WORD_S
		KEY_F24,  //GESTURE_SLIDE_UP
		KEY_POWER,  //GESTURE_SLIDE_DOWN
		KEY_POWER,  //GESTURE_SLIDE_LEFT
		KEY_POWER,  //GESTURE_SLIDE_RIGHT
};

static bool gesture_enable=0;

#endif

static unsigned int synap_psensor_state=0;
static unsigned int synap_audio_mode=0;
#ifdef CONFIG_FB
...
static struct proc_dir_entry *synap_gesture_entry;
#define SYNAP_CUSTOM_GESTURE "goodix_gesture"

long  gestures_flag= 0;


static ssize_t synap_read_proc(struct file *filp, char __user *buf,size_t count, loff_t *f_pos)
{
	ssize_t retval=0;
	...
	*f_pos += count;
	retval = sprintf(buf, "GESTURE=0x%03x\n", (unsigned int)gestures_flag);
	
	return retval;
}
static ssize_t synap_write_proc(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	int  ret=0;
    char msg[20]={0};
	...
	ret = kstrtol(msg,0,&gestures_flag);//0x
	
	
	if(ret){
		printk("\t  kstrtol fail i16z=[%d]\n",ret);
	}else{  

		if (((gestures_flag & 0x100) != 0)  || (gestures_flag & _GESTURE_DOUBLE_CLICK) || (gestures_flag & _GESTURE_SLIDE)){
...
static const struct file_operations synap_gesture_fops = {
	.owner = THIS_MODULE,
	.read = synap_read_proc,
	.write = synap_write_proc,
};

#endif
#if (_CustomFunction_ASUS_GLOVE)
static struct proc_dir_entry *synap_glove_entry=NULL;
struct synaptics_rmi4_data *glove_rmi4_data=NULL;
#define SYNAP_CUSTOM_GLOVE  "goodix_glove_switch"
#define SYNAP_GLOVE_REGISTER 0x001c
unsigned int  glove_enable = 0;
extern unsigned int  syna_firmware_update;
unsigned char glove_open_value =0x25;
unsigned char glove_disable_value =0x05;

static ssize_t synap_glove_read_proc(struct file *filp, char __user *buf,size_t count, loff_t *f_pos)
{
	ssize_t retval=0;
	if(*f_pos)
	 {
...
static ssize_t synap_glove_write_proc(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	int  ret=0;
       char msg[20]={0};
	...
	if (ret)
		...
		ret = kstrtouint(msg, 0, &glove_enable);
        ...
        if(syna_firmware_update == 1)
        {
            printk("in firmware update ,can't open glove!!!!!!\n\n\n");
            return count;
        }
		printk("glove msg =%s glove_enable =%d\n ",msg,glove_enable);
		
		if (glove_enable){ //0x001b 0x25
				ret= synaptics_rmi4_reg_write(glove_rmi4_data,SYNAP_GLOVE_REGISTER,&glove_open_value,sizeof(glove_open_value));
				if(ret < 0)
					printk("write glove enable fail %d\n ",ret);	
		} else {//0x001b 0x5
				ret=synaptics_rmi4_reg_write(glove_rmi4_data,SYNAP_GLOVE_REGISTER,&glove_disable_value,sizeof(glove_disable_value));
				if(ret < 0)
					printk("write glove disable fail %d\n ",ret);
		}

	return count;
}
static const struct file_operations synap_glove_fops = {
	.owner = THIS_MODULE,
	.read = synap_glove_read_proc,
	.write = synap_glove_write_proc,
};

#endif

static struct proc_dir_entry *synap_psensor_state_entry=NULL;
static struct proc_dir_entry *synap_audio_mode_entry=NULL; 
static ssize_t psensor_state_write_proc(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ret = kstrtouint(msg, 0, &synap_psensor_state);
	
    printk("synap psensor_state=%d\n",synap_psensor_state);
	
	return count;
}
static const struct file_operations synap_psensor_state_fops = {
	.owner = THIS_MODULE,
	.read = NULL,
	.write = psensor_state_write_proc,
};

static ssize_t synap_audio_mode_write_proc(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ret = kstrtouint(msg, 0, &synap_audio_mode);
	
    printk("synap synap_audio_mode=%d\n",synap_audio_mode);
	
	return count;
}
static const struct file_operations synap_audio_mode_fops = {
	.owner = THIS_MODULE,
	.read = NULL,
	.write = synap_audio_mode_write_proc,
};
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned char gesture_type,gesture_type_ex;
	uint32_t keycode = 0;
	unsigned short data_addr;
	int x;
	int y;
	...
	if (rmi4_data->suspend && (rmi4_data->enable_wakeup_gesture || gesture_enable)) { 
		...
		gesture_type_ex=rmi4_data->gesture_detection[2];
		
		printk("synap gesture_type=%x,gesture_type_ex=%x \n",gesture_type,gesture_type_ex);
		if (gesture_type && gesture_type != F12_UDG_DETECT) {
			if((gesture_type== 0x03)&&(gestures_flag & _GESTURE_DOUBLE_CLICK))
			{
		
				keycode = gesture_key_array[3];
			}
				
			else if ((gesture_type== 0x07) && (gestures_flag & _GESTURE_SLIDE))
			{
				keycode = gesture_key_array[9];
			}

			else{ //double,swip up,c,e,s,v,w,z
				switch (gesture_type_ex) {

				case 0x63://c
					if((gestures_flag & _GESTURE_C)==_GESTURE_C)
					{
						...
						break;
				case 0x7a://z
					if((gestures_flag & _GESTURE_Z)==_GESTURE_Z)
					{
							keycode = gesture_key_array[4];
					}
						break;
				case 0x73://s
					if((gestures_flag & _GESTURE_S)==_GESTURE_S)
					{
				...
				case 0x77://w
					if((gestures_flag & _GESTURE_W)==_GESTURE_W)
					{
						keycode = gesture_key_array[1];
					}
						break;
				case 0x2://V
					if((gestures_flag & _GESTURE_V)==_GESTURE_V)
					{
						keycode = gesture_key_array[2];
					}
						break;
				case 0x65://e
					if((gestures_flag & _GESTURE_E)==_GESTURE_E)
						{
		...
		bool report)
{
    ...
    if (synap_psensor_state && (synap_audio_mode==2))
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
  ...
  for (ii = 0; ii < (sizeof(gesture_key_array) / sizeof(gesture_key_array[0])); ii++) 
  	{
		input_set_capability(rmi4_data->input_dev, EV_KEY, gesture_key_array[ii]);
	}
#endif
		...
		bool enable)
{
	...
	if (rmi4_data->bus_reg) {
	  ...
	  retval = regulator_set_load(rmi4_data->bus_reg,bdata->regulator_i2cvdd_current);
	  
	  if (retval < 0) {
		  dev_err(rmi4_data->pdev->dev.parent,
				  "%s: Failed to set regulator i2c-current avdd\n",__func__);
		  goto exit;
	  }
...
#if (_CustomFunction_ASUS_)
	synap_gesture_entry = proc_create(SYNAP_CUSTOM_GESTURE, 0666, NULL,&synap_gesture_fops);
	if (synap_gesture_entry == NULL) {
			dev_err(&pdev->dev,"create proc/%s Failed!\n",SYNAP_CUSTOM_GESTURE);
			
		return -ENOMEM;
		
	} else {
		dev_info(&pdev->dev,"create proc//%s Succeeded!\n",SYNAP_CUSTOM_GESTURE);
	}
#endif
#if (_CustomFunction_ASUS_GLOVE)
	synap_glove_entry = proc_create(SYNAP_CUSTOM_GLOVE, 0666, NULL,&synap_glove_fops);
	if (synap_glove_entry == NULL) {
			dev_err(&pdev->dev,"create proc/%s Failed!\n",SYNAP_CUSTOM_GLOVE);
...
#endif
synap_psensor_state_entry = proc_create("psensor_state", 0666, NULL,&synap_psensor_state_fops);
	if (synap_psensor_state_entry == NULL) {
			dev_err(&pdev->dev,"create proc/psensor_state Failed!\n");
			
		return -ENOMEM;
		
	} else {
		dev_info(&pdev->dev,"create proc/psensor_state Succeeded!\n");
	}

synap_audio_mode_entry = proc_create("audio_mode", 0666, NULL,&synap_audio_mode_fops);
	if (synap_audio_mode_entry == NULL) {
			dev_err(&pdev->dev,"create proc/audio_mode  Failed!\n");
			
		return -ENOMEM;
		
	} else {		
		dev_info(&pdev->dev,"create proc/audio_mode Succeeded!\n");
...
static int synaptics_rmi4_suspend(struct device *dev)
{
    ...
    printk("%s\n",__func__);
	if (rmi4_data->enable_wakeup_gesture || gesture_enable) { 
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	{
		printk(" synaptics already resume\n");
		return 0;
	}
...
#if (_CustomFunction_ASUS_GLOVE)
	if(glove_enable){
		ret= synaptics_rmi4_reg_write(rmi4_data,SYNAP_GLOVE_REGISTER,&glove_open_value,sizeof(glove_open_value));
		if(ret < 0)
		 {
		 	printk("resume write glove enable fail %d\n ",ret);
		  }	
		}
#endif
#ifdef FB_READY_RESET
