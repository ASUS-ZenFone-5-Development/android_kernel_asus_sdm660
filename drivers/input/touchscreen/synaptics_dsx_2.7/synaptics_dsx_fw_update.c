...
unsigned int  syna_firmware_update=0;
extern unsigned int  glove_enable;
extern unsigned char glove_open_value;
...
#ifdef TP_AUTO_UPDATE_FW_MODE
static int get_boot_mode(struct synaptics_rmi4_data *rmi4_data)
{
        int ret;
        ...
        if(cmdline_tp != NULL)
        {
                ...
                dev_err(rmi4_data->pdev->dev.parent, "cmd_line =%s \n", cmd_line);
                if(ret==0)
                {
                        dev_err(rmi4_data->pdev->dev.parent, "mode: ffbm\n");
                        return 1;/* factory mode*/
                }
		else
		{
		        dev_err(rmi4_data->pdev->dev.parent, "mode: no ffbm\n");
                        return 2;/* no factory mode*/
		}
        }
       dev_err(rmi4_data->pdev->dev.parent, "has no androidboot.mode \n");
       return 0;
}
#endif
...
static enum flash_area fwu_go_nogo(void)
{
	...
	for (ii = 0; ii < config_id_size; ii++) {
		if (fwu->img.ui_config.data[ii] > fwu->config_id[ii]) {
			flash_area = UI_FIRMWARE;//UI_CONFIG
			goto exit;
...
exit:
    ...
    if(glove_enable == 1)
    {
        printk("synap enable glove after firmware_update!!!\n");
        synaptics_rmi4_reg_write(rmi4_data,0x001c,&glove_open_value,sizeof(glove_open_value));
		
    }
	syna_firmware_update = 0;
	return retval;
}
...
//	char firmware_ver[HARDWARE_MAX_ITEM_LONGTH];
    ...
    retval = fwu_get_device_config_id();
	if (retval < 0) {
			printk("%s: Failed to read device config ID  after update firmware\n",__func__);
			
		}
	
//	snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"TCL,synaptics,FW:%02x%02x%02x%02x",*(fwu->config_id),*(fwu->config_id+1),*(fwu->config_id+2),*(fwu->config_id+3));
...
//	char firmware_ver[HARDWARE_MAX_ITEM_LONGTH];
...
#ifdef TP_AUTO_UPDATE_FW_MODE
	if(get_boot_mode(rmi4_data) == 0) //nomal_boot_mode
	{

	dev_err(rmi4_data->pdev->dev.parent," normal boot mode\n");
#ifdef DO_STARTUP_FW_UPDATE
	...
	else //other boot mode
	{

	dev_err(rmi4_data->pdev->dev.parent,"not normal boot mode,not update\n");
